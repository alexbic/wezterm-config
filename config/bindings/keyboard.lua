...
local base_keys = {
   ...
   { key = "t", mods = "CTRL|SHIFT", action = wezterm.action.SpawnTab "CurrentPaneDomain", description = locale_t.open_new_tab or "Открыть новую вкладку" },
   { key = "w", mods = "CTRL|SHIFT|ALT", action = bindings_utils.create_workspace_new_window(wezterm, locale_t.enter_workspace_name_new_window or "Введите имя workspace для нового окна"), description = locale_t.create_workspace_new_window or "Создать workspace в новом окне" },
}

return {
   disable_default_key_bindings = true,
   leader = leader,
   keys = bindings_utils.build_keys(wezterm, base_keys, mod, platform, locale_t),

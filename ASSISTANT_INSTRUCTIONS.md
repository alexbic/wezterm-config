...
## 🔴 НОВЫЙ ПАТТЕРН ОШИБКИ (из чата #28)

### ИЗМЕНЕНИЕ БАЗОВОЙ АРХИТЕКТУРЫ БЕЗ АНАЛИЗА ЗАВИСИМОСТЕЙ
Проблема: Изменение структуры icons.lua на icons.t сломало все старые вызовы
Симптомы: syntax error <name> expected, attempt to index nil value
Диагностика: grep -r "старая_функция" . --include="*.lua" ПЕРЕД изменениями
Решение: Массовая замена всех зависимостей одновременно
Новое правило: ВСЕГДА анализировать зависимости перед архитектурными изменениями
...
---
*Обновлено в чате #28 - правило анализа зависимостей*

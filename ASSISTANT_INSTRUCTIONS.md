...
### 🚨 НОВЫЕ ПАТТЕРНЫ ИЗ ЧАТА #31

#### НАРУШЕНИЕ ЗОЛОТОГО ПРАВИЛА КОМАНД
Проблема: Выдача нескольких команд подряд без ожидания результата
Симптомы: Пользователь жалуется на "кашу", "не читаешь инструкции"  
Решение: СТРОГО одна команда → ждать результат → анализ → следующая команда
Проверка: Каждое сообщение = максимум ОДНА команда

### 🚨 НОВЫЕ ПАТТЕРНЫ ИЗ ЧАТА #31

#### НАРУШЕНИЕ ЗОЛОТОГО ПРАВИЛА КОМАНД (ПОВТОРНОЕ)
Проблема: Выдача сложных sed команд без ожидания результата (сообщения #11, #12)
...
Проблема: Поиск строки 59 вместо реальной строки 70 без предварительной проверки

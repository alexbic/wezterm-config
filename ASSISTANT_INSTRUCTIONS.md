...
**ФОРМАТ БЛОКА:**
# Комментарий к первой команде
команда1

# Комментарий ко второй команде  
команда2

# Комментарий к третьей команде
команда3

**ПРИНЦИПЫ:**
- Команды в блоке выполняются последовательно пользователем
- Каждая команда предваряется комментарием
- Блок завершается после получения всех результатов
- Анализ результатов происходит после выполнения всего блока

### 📊 ПРАВИЛО КРАСИВОГО ВЫВОДА РЕЗУЛЬТАТОВ
**ФОРМАТ СБОРА И ВЫВОДА:**
# Собираем результаты в переменные
RESULT1=$(команда1)
RESULT2=$(команда2) 
RESULT3=$(команда3)

# Красиво выводим собранные результаты
echo "=== ЗАГОЛОВОК ПЕРВОГО БЛОКА ==="
echo "$RESULT1"
echo ""
echo "=== ЗАГОЛОВОК ВТОРОГО БЛОКА ==="
echo "$RESULT2"

**ПРИНЦИПЫ:**
- Все результаты собираются в переменные
- Финальный вывод структурирован заголовками
- Пустые строки для разделения блоков
- Понятные описательные заголовки
...
#### НЕПРАВИЛЬНЫЕ ОБРАЩЕНИЯ К ИКОНКАМ (НОВЫЙ)
...
#### ИЗМЕНЕНИЕ БАЗОВОЙ АРХИТЕКТУРЫ БЕЗ АНАЛИЗА ЗАВИСИМОСТЕЙ (НОВЫЙ)
...
#### ОТСУТСТВИЕ ЭКСПОРТА В INIT.LUA (НОВЫЙ)
...
- Полная перестройка ASSISTANT_INSTRUCTIONS.md
...
Последнее обновление: Чат #29 - добавлена централизованная иконочная система
СИСТЕМАТИЗИРОВАННАЯ ВЕРСИЯ с полным покрытием паттернов ошибок

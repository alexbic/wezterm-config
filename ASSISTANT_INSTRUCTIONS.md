...
## 🚨 ОБНОВЛЕННЫЕ ПАТТЕРНЫ ОШИБОК (на основе диалога #1-31)

### ⚡ WEZTERM.FORMAT СПЕЦИФИЧНЫЕ ОШИБКИ
...
**Решение:** ВСЕГДА использовать одинарные: `{ { Foreground = ... }, { Text = ... } }`
...
### 📋 ОБНОВЛЕННАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ ИСПРАВЛЕНИЯ ЛОКАЛИЗАЦИИ
...
Статус: КРИТИЧНЫЕ ПАТТЕРНЫ обновлены на основе реального опыта

## 🚨 КРИТИЧЕСКИЙ ПАТТЕРН: СПЕЦСИМВОЛЫ В ECHO (НАЙДЕНО В ДИАЛОГЕ)

### 🎯 ПРОБЛЕМА CMDAND DQUOTE - ФИНАЛЬНОЕ РЕШЕНИЕ
**Причина:** Восклицательный знак и другие спецсимволы внутри кавычек echo
...
### 🛡️ ПРАВИЛО
НИКОГДА не использовать спецсимволы bash (! ` $ " ') внутри кавычек echo без экранирования

Статус: КРИТИЧЕСКИЙ ПАТТЕРН ДОКУМЕНТИРОВАН

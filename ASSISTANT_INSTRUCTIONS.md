...
### 🔴 ПРОБЛЕМЫ АВТОПЕРЕВОДА ДВОЙНЫХ ЗАПЯТЫХ
**Проблема:** translate-shell возвращает значения с кавычками, sed создает двойные запятые
**Симптомы:** `"window",",` вместо `"window",` в переведенных файлах
**Решение:** Обязательная функция clean_translation с полной очисткой кавычек
**Паттерн:** `sed 's/^["\x27„""«»]*//g' | sed 's/["\x27„""«»]*$//g'`
**Проверка:** `echo '"окно"' | trans ru:en` должен очищаться от всех кавычек

### 🔴 КРОССПЛАТФОРМЕННОСТЬ SED 
**Проблема:** macOS требует `sed -i ''`, Linux `sed -i`
**Решение:** Определять платформу и использовать условия
**Паттерн:** `[[ "$OSTYPE" == "darwin"* ]] && sed -i '' || sed -i`
...
### 🔴 ПРОБЛЕМЫ ФОРМАТА ПРОГРЕСС-БАРА  
**Требование:** Точный формат `🌐 Создание en.lua из ru.lua █████░░░ [15/47 кл.]`
**Проблема:** Локализация через fallback вместо кэша
**Решение:** Исправить функцию get_localized_message для чтения config/environment/locale.lua

...
### 🛡️ ПРАВИЛО MACOS СОВМЕСТИМОСТИ (НОВОЕ ИЗ ЧАТА #35)
- ❌ **НИКОГДА:** head -n -1 (не работает на macOS)
- ✅ **ПРАВИЛЬНО:** head -n $((total_lines - 1)) для macOS
- **ПРИЧИНА:** BSD команды отличаются от GNU Linux

### 🛡️ ПРАВИЛО F10 АРХИТЕКТУРЫ (ИЗ ЧАТА #35)
- **config/dialogs/settings-manager.lua** = ТОЛЬКО ключи локализации
- **utils/dialogs.lua** = ВСЕ функции F10 включая интеграцию
- **Недопустимо:** get_title = function() в config/dialogs/
- **Правильно:** title_key = "settings_manager_title"

### 🛡️ УСИЛЕННОЕ ПРАВИЛО ИКОНОК (ИЗ ЧАТА #35)
- ❌ **НЕДОПУСТИМО:** Любые прямые эмодзи в config/ файлах
- ✅ **ПРАВИЛЬНО:** Только icon_key = "system" с доступом через environment.icons.t.key
- **Исключение:** Fallback значения в utils/ функциях

# 🤖 ИНСТРУКЦИИ ДЛЯ AI АССИСТЕНТА - WEZTERM CONFIG PROJECT

## 👤 КОНТЕКСТ ОПЕРАТОРА
- **Имя оператора:** Александр
- **Язык общения:** Русский
- **Платформа:** macOS
- **Уровень экспертизы:** Средний (WezTerm)
- **Приоритет работы:** ЭКОНОМИЯ ТОКЕНОВ + точность решений
- **Личные предпочтения:** Любит эмодзи, дотошный, ценит порядок

## ⚙️ НАСТРОЙКИ ДИАЛОГА - ОПТИМИЗИРОВАННЫЕ
- **Тип ассистента:** Claude
- **Реальный лимит:** ~16 сообщений (НЕ 35!)
- **Порог подготовки к завершению:** 14+ сообщений (~85%)
- **Формат commit:** Подробный с историей чата
- **Стиль взаимодействия:** МАКСИМАЛЬНО СЖАТЫЕ команды + итоговый результат

## 📊 АВТОМАТИЧЕСКИЙ КОНТРОЛЬ ДИАЛОГА
**ОБЯЗАТЕЛЬНО:**
- Вести счетчик: #X/35 с первого сообщения
- При 8 сообщениях: первое предупреждение (50%)
- При 14 сообщениях: подготовка к завершению (87%)
- Обращаться по имени: "Александр"
- Адаптировать под средний уровень WezTerm
- Команды для macOS

## 🚨 НОВЫЕ ПРИОРИТЕТЫ ЭКОНОМИИ ТОКЕНОВ (КРИТИЧНО)

### 🛡️ ЗОЛОТОЕ ПРАВИЛО СЖАТОСТИ
- **АНАЛИЗ В БЛОКЕ** - диагностика + исправление в одной команде
- **НЕ ПЕРЕПИСЫВАТЬ** файлы целиком - только точечные sed/изменения
- **СЖАТЫЕ ИТОГИ** - только результат без детализации
- **ДЕТАЛЬНЫЙ РЕЖИМ** - только по запросу "дай детальный блок"

### 🛡️ ФОРМАТ БЛОКОВ (НОВЫЙ СТАНДАРТ)

#### 🟢 ОБЫЧНЫЙ РЕЖИМ (по умолчанию)
ФОРМАТ: sed + echo БЕЗ markdown блоков
sed -i "s/проблема/решение/" file.lua
echo "✅ РЕЗУЛЬТАТ: что исправлено"

#### 🟡 ДЕТАЛЬНЫЙ РЕЖИМ (только по запросу)
Развернутые объяснения + полная диагностика (только если Александр скажет "дай детальный блок")

## 🛡️ КРИТИЧЕСКИЕ ПРАВИЛА (ПРИОРИТЕТ #1)

### 🛡️ ПРАВИЛО TEE HEREDOC БЕЗОПАСНОСТИ (КРИТИЧНО!)
- ❌ **НИКОГДА:** cat > file (ломает markdown)
- ❌ **КРИТИЧНО:** Тройные кавычки внутри heredoc
- ❌ **ЗАПРЕЩЕНО:** Любые markdown блоки внутри tee
- ✅ **ПРАВИЛЬНО:** tee file > /dev/null БЕЗ markdown
- ✅ **БЕЗОПАСНО:** Только простой текст/код без форматирования
- **ПРИЧИНА:** Heredoc + markdown символы = поломка синтаксиса

### 🛡️ ПРАВИЛО ECHO СИНТАКСИСА (КРИТИЧНО)
- ❌ НИКОГДА: echo "текст!" (восклицательный знак перед кавычкой)
- ✅ ПРАВИЛЬНО: echo "текст" или echo "текст."

### 🛡️ ПРАВИЛО MACOS СОВМЕСТИМОСТИ
- ❌ **НИКОГДА:** head -n -1 (не работает на macOS)
- ✅ **ПРАВИЛЬНО:** head -n $((total_lines - 1))

### 🛡️ ПРАВИЛО АРХИТЕКТУРНОЙ ЧИСТОТЫ
- ❌ **НИКОГДА:** функции в config/ файлах
- ✅ **ПРАВИЛЬНО:** config/=данные, utils/=функции
- ❌ **НЕДОПУСТИМО:** прямые эмодзи в config/
- ✅ **ПРАВИЛЬНО:** только environment.icons.t.key

### 🛡️ ПРАВИЛО БЛОКОВ КОМАНД
- **ПРОСТЫЕ команды** = прямой вывод результата
- **СЛОЖНЫЕ команды** = сбор в переменные + итоговый анализ
- **ЖДАТЬ анализа** результата перед следующим блоком

## 📋 ПАТТЕРН ДЛЯ ПРОЕКТА WEZTERM
✅ ВСЕ скрипты = кроссплатформенные (macOS/Linux/Windows)
✅ ВСЕ настройки = с учетом локализации  
✅ ВСЕ команды sed = через cross_platform_sed функции
✅ ВСЕ создания файлов = через tee БЕЗ markdown
✅ F10 архитектура: config/dialogs/ ←→ utils/dialogs.lua
✅ Централизованные системы: иконки, цвета, локализация
✅ ПРИ НЕОПРЕДЕЛЕННОСТИ = web_search ПЕРВЫМ ДЕЛОМ

## 🎯 МЕТА-ПРАВИЛА ДЛЯ AI

### ⚠️ КРИТИЧЕСКИЕ НАПОМИНАНИЯ (НЕ ЗАБЫВАТЬ!)
- **ЭКОНОМИЯ ТОКЕНОВ ПРЕВЫШЕ ВСЕГО**
- **АНАЛИЗ ДОКУМЕНТОВ ПЕРВЫМ ДЕЛОМ** - у меня доступ к актуальным файлам
- **ОДИН БЛОК** вместо множества команд
- **БЕЗ ПЕРЕСПРАШИВАНИЙ** - если план ясен, сразу исправляем
- **CROSS_PLATFORM функции ВСЕГДА** для sed/mktemp
- **НЕ использовать markdown внутри heredoc**

### ПРИОРИТЕТЫ ПРИ КОНФЛИКТЕ ПРАВИЛ
1. **Экономия токенов** > все остальное
2. **Критические правила** > красота кода  
3. **Безопасность синтаксиса** > удобство
4. **Архитектурная чистота** > быстрые решения

### СТИЛЬ ОБЩЕНИЯ С АЛЕКСАНДРОМ (ОПТИМИЗИРОВАННЫЙ)
- Всегда обращаться по имени
- Украшать диалог эмодзи (он любит)
- ПРИОРИТЕТ: ЭКОНОМИЯ ТОКЕНОВ + точность решений
- Фиксировать прогресс кратко
- НЕ ДЕЛАТЬ повторных ошибок из предыдущих чатов

### 🛡️ НОВЫЕ ПАТТЕРНЫ ИЗ ЧАТА #38 (КРИТИЧНО):
- ❌ НЕ создавать файлы без обоснования в плане
- ❌ НЕ дублировать код в блоках команд
- ❌ НЕ превышать лимит 16 сообщений
- ✅ macOS sed: ВСЕГДА использовать -i ""
- ✅ Два типа диалогов: InputSelector + PromptInputLine
- ✅ Структура диалогов: config/data.lua + utils/functions.lua
- ✅ При завершении: инструкции→roadmap→коммит→push

### 🛡️ ПРАВИЛО ДИАЛОГОВЫХ СИСТЕМ:
- InputSelector: для списков выбора
- PromptInputLine: для ввода текста
- config/dialogs/name.lua = ТОЛЬКО данные
- utils/dialogs.lua = ТОЛЬКО функции

## 🛡️ АЛГОРИТМ ЗАВЕРШЕНИЯ ЧАТОВ (28-35 сообщений)
1. Анализ новых паттернов ошибок
2. Обновление ASSISTANT_INSTRUCTIONS.md
3. Обновление ROADMAP.md  
4. git commit + push

---
📋 СТАТУС: F10 базовая архитектура работает, переходим к единому интерфейсу состояний

## 🛡️ ПРАВИЛО MASTER ФАЙЛОВ (КРИТИЧНО)
- ❌ **НИКОГДА НЕ УДАЛЯТЬ:** *.master файлы (эталонные версии)
- ✅ **СОХРАНЯТЬ:** .master = эталонные версии для восстановления
- ✅ **УДАЛЯТЬ ТОЛЬКО:** .tmp*, .backup*, .bak, .fixed, .patch
- **ПРИЧИНА:** .master файлы - часть рабочего пространства
### ПРАВИЛО COMMIT ПОСЛЕ БЛОКОВ
- После крупных исправлений - git commit
- Формат: CHAT #X: описание

### ПРАВИЛО ЛИМИТА 16 СООБЩЕНИЙ  
- Реальный лимит: 16 (НЕ 35)
- При 14: подготовка к завершению

### 🛡️ ПАТТЕРН ЭФФЕКТИВНЫХ БЛОКОВ КОМАНД (из чата #39)

### 🛡️ ПРАВИЛО СОБЫТИЙ WEZTERM (КРИТИЧНО - из чата #40)
- ❌ **НЕ ТРОГАТЬ:** events/tab-title.lua без крайней необходимости
- ❌ **НЕ ЛОМАТЬ:** структуру if-then-else в обработчиках событий
- ❌ **НЕ ДОБАВЛЯТЬ:** номера в названия вкладок (вызывает FormatColor ошибки)
- ✅ **EMERGENCY BACKUP:** обязательно перед правкой событий
- ✅ **НЕМЕДЛЕННЫЙ ОТКАТ:** при появлении бесконечного логирования
- **ПРИЧИНА:** События tab-title критичны для UI, ошибки вызывают краш
- ✅ **ОДИН МЕГА-БЛОК:** все команды + переменные + проверка результата
- ✅ **ПЕРЕМЕННЫЕ ДЛЯ ЗАВИСИМОСТЕЙ:** ERROR_LINE, BACKUP_FILE, SYNTAX_CHECK
- ✅ **РЕЗУЛЬТАТ В КОНЦЕ:** только итоговый echo с статусом
- ✅ **БЕЗ ЛИШНИХ КОММЕНТАРИЕВ:** только необходимые пояснения
- ❌ **НЕ РАЗБИВАТЬ:** на множество мелких блоков

### 🛡️ ПРАВИЛО КОНТРОЛЯ СООБЩЕНИЙ (КРИТИЧНО)
- **ОБЯЗАТЕЛЬНО:** указывать #X/16 в каждом ответе
- **При 10+ сообщениях:** переходить к завершению
- **При 14+ сообщениях:** немедленное завершение

### 🛡️ НОВЫЕ КРИТИЧЕСКИЕ ПРАВИЛА ИЗ ЧАТА #41:

### 🛡️ ПРАВИЛА ИЗ ЧАТА #42:

### 🛡️ ПРАВИЛА ИЗ ЧАТА #44:
- ❌ **НЕ ИСПОЛЬЗОВАТЬ** сложные sed на многострочных callback функциях
- ✅ **ТОЛЬКО** полная перезапись при правке биндингов keyboard.lua
- ✅ **АНАЛИЗИРОВАТЬ** существующие ключи локализации перед созданием config
- ✅ **НЕ СОЗДАВАТЬ** избыточные тестовые файлы - использовать существующие
- **УРОК:** Проверка locale ключей критична перед созданием dialog config- ❌ **НЕ ДОБАВЛЯТЬ ФУНКЦИИ ПО ЧАСТЯМ** - делать все сразу в одном блоке
- ❌ **НЕ ИСПОЛЬЗОВАТЬ echo с ! перед кавычкой** (bash ошибка)
- ❌ **НЕ СОЗДАВАТЬ НОВЫЕ ФАЙЛЫ** когда можно улучшить существующие
- ✅ **ВСЕГДА ПЕРЕЗАПИСЫВАТЬ ЦЕЛИКОМ** проблемные файлы из документов
- ✅ **ИСПОЛЬЗОВАТЬ СУЩЕСТВУЮЩУЮ АРХИТЕКТУРУ** вместо создания дублей
- ✅ **ЭКСТРЕННАЯ ОЧИСТКА** - приоритет перед новыми функциями
- **УРОК:** Анализ хаоса критичен перед началом разработки
- ❌ **НЕ ТРОГАТЬ** events/tab-title.lua без emergency backup + понимания if-then-else структуры
- ❌ **НЕ ИСПОЛЬЗОВАТЬ** сложные sed блоки для событий (вызывают синтаксические ошибки)
- ❌ **НЕ НАРУШАТЬ** структуру функций в событиях
- ✅ **ИСПОЛЬЗОВАТЬ** официальные input_selector_label_bg/fg цвета для InputSelector
- ✅ **ВСЕГДА** проверять luac -p после КАЖДОГО изменения событий
- ✅ **ЭКСТРЕННЫЙ ОТКАТ** при любых ошибках в событиях

### 🛡️ ПРАВИЛО INPUTSELECTOR ЦВЕТОВ:

### 🛡️ ПАТТЕРН БЕЗОПАСНОЙ ВСТАВКИ В ФАЙЛЫ (из чата #43):
```bash
# 1. АНАЛИЗ СТРУКТУРЫ ФАЙЛА
TARGET_FILE="filename.md"
grep -n "^## " "$TARGET_FILE" | tail -3  # показать последние секции

# 2. ПОИСК ТОЧКИ ВСТАВКИ (несколько fallback вариантов)
INSERT_POINT=$(grep -n "нужный_ключ" "$TARGET_FILE" | cut -d: -f1)
if [ -z "$INSERT_POINT" ]; then
  INSERT_POINT=$(grep -n "запасной_ключ" "$TARGET_FILE" | cut -d: -f1)
fi
if [ -z "$INSERT_POINT" ]; then
  INSERT_POINT=$(wc -l < "$TARGET_FILE")  # в конец файла
fi

# 3. БЕЗОПАСНАЯ ПРОВЕРКА И ВСТАВКА
if [ ! -z "$INSERT_POINT" ] && [ "$INSERT_POINT" -gt 0 ]; then
  sed -i "" "${INSERT_POINT}a\\новый_контент" "$TARGET_FILE"
  echo "✅ Успешно вставлено в строку $INSERT_POINT"
else
  echo "❌ Ошибка: точка вставки не найдена"
fi
```
- ✅ **ВСЕГДА** анализировать структуру перед вставкой
- ✅ **НЕСКОЛЬКО fallback** вариантов для точки вставки
- ✅ **ПРОВЕРКА** валидности точки вставки
- ✅ **ИНФОРМАТИВНЫЕ** сообщения о результате
- ✅ Официальные цвета: input_selector_label_bg/fg в colors секции
- ❌ Hover-эффекты НЕ ПОДДЕРЖИВАЮТСЯ (ограничение WezTerm API)
- ✅ Только статическое цветовое оформление через wezterm.format
